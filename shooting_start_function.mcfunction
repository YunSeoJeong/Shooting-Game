...
#effect
execute as @a[tag=p1,tag=shooter,limit=1,scores={storage=..0}] run effect give @a[tag=p2,limit=1] minecraft:instant_damage 1 0 true
execute as @a[tag=p2,tag=shooter,limit=1,scores={storage=..0}] run effect give @a[tag=p1,limit=1] minecraft:instant_damage 1 0 true
